...
def get_telem_data():
    ...
    northern_tx = np.vectorize(haversine)(Data[:,2], Data[:,3],136.54462046921253, -30.92590749591904)
    western_tx = np.vectorize(haversine)(Data[:,2], Data[:,3], 136.5386736765504, -30.927738983922794)
    southern_tx = np.vectorize(haversine)(Data[:,2], Data[:,3], 136.54678132385015,-30.934927407856446)
    gcs_tx = np.vectorize(haversine)(Data[:,2], Data[:,3], 136.54463,-30.93484)
