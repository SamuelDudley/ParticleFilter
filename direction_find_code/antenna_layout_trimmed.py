...
def phase_calc(u,v,p1,p2,lamba):
    ...
    b = np.sqrt((np.cos(u)*np.sin(v))**2.0 + (np.sin(u)*np.sin(v))**2.0 + (np.cos(v))**2.0) # turns out this is always 1...
    D1 = a/1.
    return D1 / lamba
...
point2  = np.array([0.15,0.00, 0.000]) # two of three in a 'linear array'
point3 = np.array([0.22, 0.00, 0.000]) # three of three in a 'linear array'
point4 = np.array([0.220, 0.22,0.22000]) # extra antenna used to make unique soln's

c = 299792458 # speed of light [m/s]
frequency = 18.*10**9.0 # 1.3 GHz
...
u, v = np.mgrid[0.00001:2*np.pi:300j, 0.00001:np.pi:300j]
u_measured, v_measured = np.mgrid[np.radians(azimuth):np.radians(azimuth):2j, np.radians(elevation):np.radians(elevation):2j]

z1_measured = phase_calc(u_measured,v_measured,point1,point2,lamba)
z2_measured = phase_calc(u_measured,v_measured,point1,point3,lamba)
z3_measured = phase_calc(u_measured,v_measured,point1,point4,lamba)
...
# fig = plt.figure()
# ax = fig.gca(projection='3d')
# ax.plot_surface(*srf1.xyzs,color="g")
# plt.show()
