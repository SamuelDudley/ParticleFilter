...
class Map(object):
    ...
    def draw_pyvis(self):
        ...
        color = (0.8, 0.25, 0.)
        fig[0:4, 0:4].plot(points, width=0, face_color=color + (0.05,), edge_color=None)
        fig.show(run=True)
    ...
    def draw_thread(self):
        self.t = threading.Thread(target=self.draw_pyvis, name='log_writer')
        self.t.daemon = True
        self.t.start()
