...
class Map(object):
    ...
    def update(self):
        for particle in self.particles:
            ...
            distance = self.modelHelperFunctions.x_dist(self.transmitters[0].state.x, particle.state.x)
        ...
        distance = self.modelHelperFunctions.x_dist(self.transmitters[0].state.x, self.aircrafts[0].state.x)
        frequency = self.transmitters[0].state.frequency
        transmitterHeight = self.transmitters[0].state.z
        transmitterPower = self.transmitters[0].state.power
    ...
    def assess(self):
        ...
        while len(new_particles) < self.numberParticles:
            ...
            if p is None:  # No pick b/c all totally improbable
                ...
                #p.state.y += np.random.normal(0, 5, 1)[0] #randomise the y location of the particle
    ...
    def step(self):
        #move the aircraft
        self.aircrafts[0].state.x += self.timeDelta*self.aircrafts[0].state.groundSpeed
        
        #move the particles
        for particle in self.particles:
            particle.state.x += self.timeDelta*particle.state.groundSpeed
