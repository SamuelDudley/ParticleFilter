...
import rf_data_wtr_x8
import ap_telem_data_wtr_x8
import Model

def subtract(a,b):
    return a-b

rf = rf_data_wtr_x8.get_rf_data()
ap = ap_telem_data_wtr_x8.get_telem_data()
...
x_ap = ap[:,0]
alt_ap = ap[:,4]
alt_ap = np.vectorize(subtract)(alt_ap,136.)


#NORTH, WEST, SOUTH
north = {'rf':6, 'ap':-3, 'freq':1280, 'height':3.2}
west = {'rf':59, 'ap':-2, 'freq':1320, 'height':3.2}
south = {'rf':112, 'ap':-1, 'freq':1360, 'height':3.4}
...
two_ray = np.vectorize(Model.twoRay)(ap[:,transmitter['ap']], transmitter['freq'], transmitter['height'], alt_ap, 40)
